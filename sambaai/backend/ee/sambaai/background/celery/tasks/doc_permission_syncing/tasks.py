...
from ee.sambaai.configs.app_configs import DEFAULT_PERMISSION_DOC_SYNC_FREQUENCY
from ee.sambaai.db.connector_credential_pair import get_all_auto_sync_cc_pairs
from ee.sambaai.db.document import upsert_document_external_perms
from ee.sambaai.external_permissions.sync_params import DOC_PERMISSION_SYNC_PERIODS
...
from sambaai.access.models import DocExternalAccess
...
from sambaai.background.celery.celery_redis import celery_get_queue_length
from sambaai.background.celery.celery_redis import celery_get_queued_task_ids
...
from sambaai.configs.constants import CELERY_PERMISSIONS_SYNC_LOCK_TIMEOUT
from sambaai.configs.constants import CELERY_TASK_WAIT_FOR_FENCE_TIMEOUT
from sambaai.configs.constants import SAMBAAI_REDIS_FUNCTION_LOCK_PREFIX
...
from sambaai.db.connector import mark_cc_pair_as_permissions_synced
from sambaai.db.connector_credential_pair import get_connector_credential_pair_from_id
from sambaai.db.document import get_document_ids_for_connector_credential_pair
...
from sambaai.db.utils import is_retryable_sqlalchemy_error
...
from sambaai.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSync
from sambaai.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSyncPayload
...
from sambaai.utils.logger import doc_permission_sync_ctx
from sambaai.utils.logger import format_error_for_logging
from sambaai.utils.logger import LoggerContextVars
...
def _is_external_doc_permissions_sync_due(cc_pair: ConnectorCredentialPair) -> bool:
    ...
    source_sync_period *= int(SambaAIRuntime.get_doc_permission_sync_multiplier())
...
@shared_task(
    name=SambaAICeleryTask.CHECK_FOR_DOC_PERMISSIONS_SYNC,
    ignore_result=True,
...
def check_for_doc_permissions_sync(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        SambaAIRedisLocks.CHECK_CONNECTOR_DOC_PERMISSIONS_SYNC_BEAT_LOCK,
    ...
    try:
        ...
        lock_beat.reacquire()
        if not r.exists(SambaAIRedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES):
            ...
            r.set(SambaAIRedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES, 1, ex=300)
        ...
        for key in keys:
            ...
            if not r.exists(key_bytes):
                r.srem(SambaAIRedisConstants.ACTIVE_FENCES, key_bytes)
                continue
...
def try_creating_permissions_sync_task(
    ...
    lock: RedisLock = r.lock(
        SAMBAAI_REDIS_FUNCTION_LOCK_PREFIX + "try_generate_permissions_sync_tasks",
    ...
    try:
        ...
        result = app.send_task(
            SambaAICeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair_id,
                tenant_id=tenant_id,
            ),
            queue=SambaAICeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
...
@shared_task(
    name=SambaAICeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_permission_sync_generator_task(
    ...
    lock: RedisLock = r.lock(
        SambaAIRedisLocks.CONNECTOR_DOC_PERMISSIONS_SYNC_LOCK_PREFIX
...
# @shared_task(
#     name=SambaAICeleryTask.UPDATE_EXTERNAL_DOCUMENT_PERMISSIONS_TASK,
...
#     completion_status = SambaAICeleryTaskCompletionStatus.UNDEFINED
...
#         completion_status = SambaAICeleryTaskCompletionStatus.SUCCEEDED
#     except Exception as e:
#         error_msg = format_error_for_logging(e)
#         task_logger.warning(
...
#             f"connector_id={connector_id} doc_id={doc_id}"
#         )
#         completion_status = SambaAICeleryTaskCompletionStatus.NON_RETRYABLE_EXCEPTION
#     finally:
#         task_logger.info(
#             f"update_external_document_permissions_task completed: status={completion_status.value} doc={doc_id}"
#         )

#     if completion_status != SambaAICeleryTaskCompletionStatus.SUCCEEDED:
#         return False
...
def validate_permission_sync_fences(
    ...
    queue_len = celery_get_queue_length(
        SambaAICeleryQueues.DOC_PERMISSIONS_UPSERT, r_celery
    )
    ...
    reserved_generator_tasks = celery_get_unacked_task_ids(
        SambaAICeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC, r_celery
    )
...
def validate_permission_sync_fence(
    ...
    found = celery_find_task(
        ...
        SambaAICeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        r_celery,
    )
    if found:
