...
from ee.sambaai.server.tenants.provisioning import setup_tenant
from ee.sambaai.server.tenants.schema_management import create_schema_if_not_exists
from ee.sambaai.server.tenants.schema_management import get_current_alembic_version
from sambaai.background.celery.apps.app_base import task_logger
from sambaai.configs.app_configs import TARGET_AVAILABLE_TENANTS
...
@shared_task(
    name=SambaAICeleryTask.CLOUD_CHECK_AVAILABLE_TENANTS,
...
def check_available_tenants(self: Task) -> None:
    ...
    lock_check: RedisLock = r.lock(
        SambaAIRedisLocks.CHECK_AVAILABLE_TENANTS_LOCK,
...
def pre_provision_tenant() -> None:
    ...
    lock_provision: RedisLock = r.lock(
        SambaAIRedisLocks.CLOUD_PRE_PROVISION_TENANT_LOCK,
        timeout=_TENANT_PROVISIONING_SOFT_TIME_LIMIT,
    )
    ...
    try:
        ...
        if tenant_id:
            ...
            try:
                from ee.sambaai.server.tenants.provisioning import (
