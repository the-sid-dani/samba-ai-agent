...
from ee.sambaai.background.celery_utils import should_perform_chat_ttl_check
from ee.sambaai.background.task_name_builders import name_chat_ttl_task
from ee.sambaai.server.reporting.usage_export_generation import create_new_usage_report
from sambaai.background.celery.apps.primary import celery_app
...
from sambaai.db.chat import get_chat_sessions_older_than
...
@shared_task(
    name=SambaAICeleryTask.PERFORM_TTL_MANAGEMENT_TASK,
    ignore_result=True,
...
@celery_app.task(
    name=SambaAICeleryTask.CHECK_TTL_MANAGEMENT_TASK,
    ignore_result=True,
    soft_time_limit=JOB_TIMEOUT,
)
...
@celery_app.task(
    name=SambaAICeleryTask.AUTOGENERATE_USAGE_REPORT_TASK,
    ignore_result=True,
    soft_time_limit=JOB_TIMEOUT,
)
...
celery_app.autodiscover_tasks(
    [
        ...
        "ee.sambaai.background.celery.tasks.cloud",
    ]
)
