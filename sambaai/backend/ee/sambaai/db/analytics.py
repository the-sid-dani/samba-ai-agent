...
from sambaai.db.models import ChatMessageFeedback
...
def fetch_sambaaibot_analytics(
    start: datetime.datetime,
    end: datetime.datetime,
    db_session: Session,
    ...
    Number of SambaAIBot Queries (Chat Sessions)
    Number of instances of Negative feedback OR Needing additional help
        (only counting the last feedback)
    """
    # Get every chat session in the time range which is a SambaAIbot flow
    ...
    subquery_first_ai_response = (
        ...
        .where(
            ...
            ChatSession.sambaaibot_flow.is_(True),
        )
        .where(
    ...
    results = (
        db_session.query(
            ...
            # feedback aren't counted against SambaAIbot
            func.sum(
                case(
                    (
                        or_(
        ...
        # message of the chat session where the chat session is SambaAIbot type and within the time
