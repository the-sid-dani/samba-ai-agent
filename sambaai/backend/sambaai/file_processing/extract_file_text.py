...
from sambaai.file_processing.unstructured import unstructured_to_text
from sambaai.file_store.file_store import FileStore
...
class SambaAIExtensionType(IntFlag):
    Plain = auto()
    Document = auto()
    Multimedia = auto()
...
def is_accepted_file_ext(ext: str, ext_type: SambaAIExtensionType) -> bool:
    if ext_type & SambaAIExtensionType.Plain:
        if ext in ACCEPTED_PLAIN_TEXT_FILE_EXTENSIONS:
            return True

    if ext_type & SambaAIExtensionType.Document:
        if ext in ACCEPTED_DOCUMENT_FILE_EXTENSIONS:
            return True

    if ext_type & SambaAIExtensionType.Multimedia:
...
def _extract_sambaai_metadata(line: str) -> dict | None:
    """
...
def read_text_file(
    ...
    encoding: str = "utf-8",
    errors: str = "replace",
    ignore_sambaai_metadata: bool = True,
) -> tuple[str, dict]:
    """
    For plain text files. Optionally extracts SambaAI metadata from the first line.
    """
    metadata = {}
    file_content_raw = ""
    for ind, line in enumerate(file):
        ...
        if ind == 0 and not ignore_sambaai_metadata:
            potential_meta = _extract_sambaai_metadata(line)
    ...
    try:
        ...
        if is_text_file_extension(file_name):
            ...
            text_content_raw, file_metadata = read_text_file(
                file, encoding=encoding, ignore_sambaai_metadata=False
            )
            return ExtractionResult(
