...
from sambaai.configs.constants import DocumentIndexType
from sambaai.configs.constants import QueryHistoryType
from sambaai.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
from sambaai.prompts.image_analysis import DEFAULT_IMAGE_ANALYSIS_SYSTEM_PROMPT
from sambaai.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_SYSTEM_PROMPT
from sambaai.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_USER_PROMPT
...
# NOTE: if you are having problems accessing the SambaAI web UI locally (especially
...
# and is available in SambaAI EE
...
# restrict access to SambaAI to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict SambaAI
...
# If set, SambaAI will listen to the `expires_at` returned by the identity
...
# DB Configs
#####
DOCUMENT_INDEX_NAME = "sambaai_index"
...
VESPA_DEPLOYMENT_ZIP = (
    os.environ.get("VESPA_DEPLOYMENT_ZIP") or "/app/sambaai/vespa-app.zip"
)
...
# View the list here:
# https://github.com/sambaai-dot-app/sambaai/blob/main/backend/sambaai/connectors/factory.py
...
# Logs SambaAI only model interactions like prompts, responses, messages etc.
LOG_SAMBAAI_MODEL_INTERACTIONS = (
    os.environ.get("LOG_SAMBAAI_MODEL_INTERACTIONS", "").lower() == "true"
)
...
# founders@sambaai.app OR message Chris Weaver or Yuhong Sun in the SambaAI
# Slack community (https://join.slack.com/t/sambaai/shared_invite/zt-1w76msxmd-HJHLe3KNFIAIzk_0dSOKaQ)
