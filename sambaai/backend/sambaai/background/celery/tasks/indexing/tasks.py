...
from sambaai.background.celery.celery_utils import httpx_init_vespa_pool
from sambaai.background.celery.memory_monitoring import emit_process_memory
from sambaai.background.celery.tasks.indexing.utils import get_unfenced_index_attempt_ids
from sambaai.background.celery.tasks.indexing.utils import IndexingCallback
from sambaai.background.celery.tasks.indexing.utils import is_in_repeated_error_state
from sambaai.background.celery.tasks.indexing.utils import should_index
from sambaai.background.celery.tasks.indexing.utils import try_creating_indexing_task
from sambaai.background.celery.tasks.indexing.utils import validate_indexing_fences
from sambaai.background.indexing.checkpointing_utils import cleanup_checkpoint
from sambaai.background.indexing.checkpointing_utils import (
    get_index_attempts_with_old_checkpoints,
)
from sambaai.background.indexing.job_client import SimpleJob
from sambaai.background.indexing.job_client import SimpleJobClient
from sambaai.background.indexing.job_client import SimpleJobException
from sambaai.background.indexing.run_indexing import run_indexing_entrypoint
from sambaai.configs.app_configs import MANAGED_VESPA
from sambaai.configs.app_configs import VESPA_CLOUD_CERT_PATH
from sambaai.configs.app_configs import VESPA_CLOUD_KEY_PATH
from sambaai.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from sambaai.configs.constants import CELERY_INDEXING_LOCK_TIMEOUT
...
from sambaai.db.connector_credential_pair import fetch_connector_credential_pairs
from sambaai.db.connector_credential_pair import get_connector_credential_pair_from_id
from sambaai.db.connector_credential_pair import set_cc_pair_repeated_error_state
...
from sambaai.db.enums import IndexingStatus
from sambaai.db.index_attempt import get_index_attempt
from sambaai.db.index_attempt import mark_attempt_canceled
from sambaai.db.index_attempt import mark_attempt_failed
from sambaai.db.search_settings import get_active_search_settings_list
from sambaai.db.search_settings import get_current_search_settings
from sambaai.db.swap_index import check_and_perform_index_swap
...
from sambaai.redis.redis_connector_index import RedisConnectorIndex
...
from sambaai.redis.redis_pool import SCAN_ITER_COUNT_DEFAULT
from sambaai.redis.redis_utils import is_fence
...
@shared_task(
    name=SambaAICeleryTask.CHECK_FOR_INDEXING,
    soft_time_limit=300,
    bind=True,
)
def check_for_indexing(self: Task, *, tenant_id: str) -> int | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        SambaAIRedisLocks.CHECK_INDEXING_BEAT_LOCK,
    ...
    try:
        ...
        if not redis_client.exists(SambaAIRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE):
            ...
            for key_bytes in redis_client_replica.scan_iter(
                ...
                if is_fence(key_bytes) and not redis_client.sismember(
                    SambaAIRedisConstants.ACTIVE_FENCES, key_bytes
                ):
                    logger.warning(f"Adding {key_bytes} to the lookup table.")
                    redis_client.sadd(SambaAIRedisConstants.ACTIVE_FENCES, key_bytes)

            redis_client.set(
                SambaAIRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE,
                1,
                ex=SambaAIRuntime.get_build_fence_lookup_table_interval(),
            )
        ...
        lock_beat.reacquire()
        # we want to run this less frequently than the overall task
        if not redis_client.exists(SambaAIRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(SambaAIRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES, 1, ex=60)

        # 3/3: FINALIZE
        lock_beat.reacquire()
        keys = cast(
            set[Any], redis_client_replica.smembers(SambaAIRedisConstants.ACTIVE_FENCES)
        )
        for key in keys:
            key_bytes = cast(bytes, key)

            if not redis_client.exists(key_bytes):
                redis_client.srem(SambaAIRedisConstants.ACTIVE_FENCES, key_bytes)
                continue
...
@shared_task(
    name=SambaAICeleryTask.CONNECTOR_INDEXING_PROXY_TASK,
    bind=True,
    acks_late=False,
    track_started=True,
)
...
# primary
@shared_task(
    name=SambaAICeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_checkpoint_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        SambaAIRedisLocks.CHECK_CHECKPOINT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for attempt in old_attempts:
                ...
                self.app.send_task(
                    SambaAICeleryTask.CLEANUP_CHECKPOINT,
                    kwargs={
                        "index_attempt_id": attempt.id,
                        "tenant_id": tenant_id,
                    },
                    queue=SambaAICeleryQueues.CHECKPOINT_CLEANUP,
...
# light worker
@shared_task(
    name=SambaAICeleryTask.CLEANUP_CHECKPOINT,
    bind=True,
)
