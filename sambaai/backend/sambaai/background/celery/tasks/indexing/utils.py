...
from sambaai.db.index_attempt import create_index_attempt
from sambaai.db.index_attempt import delete_index_attempt
from sambaai.db.index_attempt import get_all_index_attempts_by_status
from sambaai.db.index_attempt import get_index_attempt
from sambaai.db.index_attempt import get_last_attempt_for_cc_pair
from sambaai.db.index_attempt import get_recent_attempts_for_cc_pair
...
from sambaai.redis.redis_connector_index import RedisConnectorIndexPayload
...
def validate_indexing_fence(
    ...
    found = celery_find_task(
        payload.celery_task_id, SambaAICeleryQueues.CONNECTOR_INDEXING, r_celery
    )
    if found:
...
def try_creating_indexing_task(
    ...
    lock: RedisLock = r.lock(
        SAMBAAI_REDIS_FUNCTION_LOCK_PREFIX + "try_creating_indexing_task",
    ...
    try:
        ...
        queue = (
            SambaAICeleryQueues.USER_FILES_INDEXING
            if cc_pair.is_user_file
            else SambaAICeleryQueues.CONNECTOR_INDEXING
        )

        # when the task is sent, we have yet to finish setting up the fence
        # therefore, the task must contain code that blocks until the fence is ready
        result = celery_app.send_task(
            SambaAICeleryTask.CONNECTOR_INDEXING_PROXY_TASK,
            kwargs=dict(
