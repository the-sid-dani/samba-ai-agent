...
from sambaai.background.celery.celery_utils import extract_ids_from_runnable_connector
from sambaai.background.celery.tasks.indexing.utils import IndexingCallbackBase
from sambaai.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING
...
from sambaai.configs.constants import CELERY_PRUNING_LOCK_TIMEOUT
...
from sambaai.db.connector import mark_ccpair_as_pruned
...
from sambaai.db.document import get_documents_for_connector_credential_pair
...
from sambaai.db.tag import delete_orphan_tags__no_commit
from sambaai.redis.redis_connector import RedisConnector
from sambaai.redis.redis_connector_prune import RedisConnectorPrune
from sambaai.redis.redis_connector_prune import RedisConnectorPrunePayload
...
from sambaai.utils.logger import pruning_ctx
...
@shared_task(
    name=SambaAICeleryTask.CHECK_FOR_PRUNING,
    ignore_result=True,
...
def check_for_pruning(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        SambaAIRedisLocks.CHECK_PRUNE_BEAT_LOCK,
    ...
    try:
        ...
        if not r.exists(SambaAIRedisSignals.BLOCK_PRUNING):
            ...
            r.set(SambaAIRedisSignals.BLOCK_PRUNING, 1, ex=3600)

        # we want to run this less frequently than the overall task
        lock_beat.reacquire()
        if not r.exists(SambaAIRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES):
            ...
            r.set(SambaAIRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES, 1, ex=300)
...
def try_creating_prune_generator_task(
    ...
    lock: RedisLock = r.lock(
        SAMBAAI_REDIS_FUNCTION_LOCK_PREFIX + "try_creating_prune_generator_task",
    ...
    try:
        ...
        result = celery_app.send_task(
            SambaAICeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
            kwargs=dict(
            ...
            queue=SambaAICeleryQueues.CONNECTOR_PRUNING,
...
@shared_task(
    name=SambaAICeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_pruning_generator_task(
    ...
    lock: RedisLock = r.lock(
        SambaAIRedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.id}",
...
def validate_pruning_fences(
    ...
    queue_len = celery_get_queue_length(SambaAICeleryQueues.CONNECTOR_DELETION, r_celery)
...
def validate_pruning_fence(
    ...
    found = celery_find_task(
        ...
        SambaAICeleryQueues.CONNECTOR_PRUNING,
        r_celery,
    )
    if found:
