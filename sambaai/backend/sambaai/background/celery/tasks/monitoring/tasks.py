...
from sambaai.db.engine import get_db_current_time
...
from sambaai.db.models import SyncRecord
...
from sambaai.utils.logger import is_running_in_container
...
def _collect_queue_metrics(redis_celery: Redis) -> list[Metric]:
    ...
    queue_mappings = {
        ...
        "permissions_sync_queue_length": SambaAICeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        "external_group_sync_queue_length": SambaAICeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
        "permissions_upsert_queue_length": SambaAICeleryQueues.DOC_PERMISSIONS_UPSERT,
    }
...
@shared_task(
    name=SambaAICeleryTask.MONITOR_BACKGROUND_PROCESSES,
    ignore_result=True,
...
def monitor_background_processes(self: Task, *, tenant_id: str) -> None:
    ...
    lock_monitoring: RedisLock = r.lock(
        SambaAIRedisLocks.MONITOR_BACKGROUND_PROCESSES_LOCK,
...
@shared_task(
    name=SambaAICeleryTask.CLOUD_MONITOR_ALEMBIC,
)
def cloud_check_alembic() -> bool | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        SambaAIRedisLocks.CLOUD_CHECK_ALEMBIC_BEAT_LOCK,
...
@shared_task(
    name=SambaAICeleryTask.CLOUD_MONITOR_CELERY_QUEUES, ignore_result=True, bind=True
)
...
@shared_task(name=SambaAICeleryTask.MONITOR_CELERY_QUEUES, ignore_result=True, bind=True)
...
def monitor_celery_queues_helper(
    ...
    n_indexing = celery_get_queue_length(SambaAICeleryQueues.CONNECTOR_INDEXING, r_celery)
    n_sync = celery_get_queue_length(SambaAICeleryQueues.VESPA_METADATA_SYNC, r_celery)
    n_deletion = celery_get_queue_length(SambaAICeleryQueues.CONNECTOR_DELETION, r_celery)
    n_pruning = celery_get_queue_length(SambaAICeleryQueues.CONNECTOR_PRUNING, r_celery)
    ...
    n_indexing_prefetched = celery_get_unacked_task_ids(
        SambaAICeleryQueues.CONNECTOR_INDEXING, r_celery
    )
...
@shared_task(
    name=SambaAICeleryTask.MONITOR_PROCESS_MEMORY,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=SambaAICeleryQueues.MONITORING,
...
@shared_task(
    name=SambaAICeleryTask.CLOUD_MONITOR_CELERY_PIDBOX, ignore_result=True, bind=True
)
