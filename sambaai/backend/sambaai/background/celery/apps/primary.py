...
from sambaai.background.celery.celery_utils import celery_is_worker_primary
from sambaai.background.celery.tasks.indexing.utils import (
    get_unfenced_index_attempt_ids,
)
from sambaai.configs.constants import CELERY_PRIMARY_WORKER_LOCK_TIMEOUT
from sambaai.configs.constants import SambaAIRedisConstants
from sambaai.configs.constants import SambaAIRedisLocks
from sambaai.configs.constants import POSTGRES_CELERY_WORKER_PRIMARY_APP_NAME
...
from sambaai.redis.redis_connector_delete import RedisConnectorDelete
...
from sambaai.redis.redis_connector_stop import RedisConnectorStop
...
celery_app.config_from_object("sambaai.background.celery.configs.primary")
...
def on_worker_init(sender: Worker, **kwargs: Any) -> None:
    ...
    r.delete(SambaAIRedisLocks.PRIMARY_WORKER)
    ...
    lock: RedisLock = r.lock(
        SambaAIRedisLocks.PRIMARY_WORKER,
    ...
    r.delete(SambaAIRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK)

    r.delete(SambaAIRedisConstants.ACTIVE_FENCES)
...
celery_app.autodiscover_tasks(
    [
        "sambaai.background.celery.tasks.connector_deletion",
        "sambaai.background.celery.tasks.indexing",
        "sambaai.background.celery.tasks.periodic",
        "sambaai.background.celery.tasks.pruning",
        "sambaai.background.celery.tasks.shared",
        "sambaai.background.celery.tasks.vespa",
        "sambaai.background.celery.tasks.llm_model_update",
        "sambaai.background.celery.tasks.user_file_folder_sync",
    ]
)
