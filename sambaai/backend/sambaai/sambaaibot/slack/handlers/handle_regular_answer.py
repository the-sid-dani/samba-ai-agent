...
from sambaai.chat.chat_utils import prepare_chat_message_request
from sambaai.chat.models import ChatSambaAIBotResponse
from sambaai.chat.process_message import gather_stream_for_slack
from sambaai.chat.process_message import stream_chat_message_objects
from sambaai.configs.app_configs import DISABLE_GENERATIVE_AI
from sambaai.configs.constants import DEFAULT_PERSONA_ID
from sambaai.configs.sambaaibot_configs import SAMBAAI_BOT_DISABLE_DOCS_ONLY_ANSWER
from sambaai.configs.sambaaibot_configs import SAMBAAI_BOT_DISPLAY_ERROR_MSGS
...
from sambaai.configs.sambaaibot_configs import SAMBAAI_REACT_EMOJI
from sambaai.configs.sambaaibot_configs import MAX_THREAD_CONTEXT_PERCENTAGE
...
from sambaai.db.persona import persona_has_search_tool
from sambaai.db.users import get_user_by_email
from sambaai.sambaaibot.slack.blocks import build_slack_response_blocks
from sambaai.sambaaibot.slack.handlers.utils import send_team_member_message
from sambaai.sambaaibot.slack.handlers.utils import slackify_message_thread
...
from sambaai.sambaaibot.slack.utils import SlackRateLimiter
...
def handle_regular_answer(
    ...
    num_retries: int = SAMBAAI_BOT_NUM_RETRIES,
    thread_context_percent: float = MAX_THREAD_CONTEXT_PERCENTAGE,
    should_respond_with_error_msgs: bool = SAMBAAI_BOT_DISPLAY_ERROR_MSGS,
    disable_docs_only_answer: bool = SAMBAAI_BOT_DISABLE_DOCS_ONLY_ANSWER,
) -> bool:
    ...
    # the message is a dm with the SambaAI bot.
    send_as_ephemeral = (
    ...
    # or the message is a dm to the SambaAI bot, we should use the proper sambaai user from the email.
    # This will make documents privately accessible to the user available to SambaAI Bot answers.
    # Otherwise - if not ephemeral or DM to SambaAI Bot - we must use None as the user to restrict
    # to public docs.
    ...
    if not message_ts_to_respond_to and not is_bot_msg:
        # if the message is not "/sambaai" command, then it should have a message ts to respond to
        raise RuntimeError(
    ...
    try:
        ...
        answer = _get_slack_answer(
            ...
            sambaai_user=user if can_search_over_private_docs else None,
        )
    ...
    if not answer.answer and disable_docs_only_answer:
        logger.notice(
            ...
            "`SAMBAAI_BOT_DISABLE_DOCS_ONLY_ANSWER` env variable is set"
        )
        return True
    ...
    try:
        ...
        # For DM (ephemeral message), we need to create a thread via a normal message so the user can see
        # the ephemeral message. This also will give the user a notification which ephemeral message does not.
        # if there is no message_ts_to_respond_to, and we have made it this far, then this is a /sambaai message
