...
from sambaai.configs.app_configs import DISABLE_TELEMETRY
from sambaai.configs.constants import ID_SEPARATOR
from sambaai.configs.constants import MessageType
from sambaai.configs.sambaaibot_configs import SAMBAAI_BOT_FEEDBACK_VISIBILITY
from sambaai.configs.sambaaibot_configs import SAMBAAI_BOT_MAX_QPM
from sambaai.configs.sambaaibot_configs import SAMBAAI_BOT_MAX_WAIT_TIME
from sambaai.configs.sambaaibot_configs import SAMBAAI_BOT_NUM_RETRIES
from sambaai.configs.sambaaibot_configs import (
    SAMBAAI_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD,
)
from sambaai.configs.sambaaibot_configs import (
    SAMBAAI_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS,
)
from sambaai.connectors.slack.utils import SlackTextCleaner
...
from sambaai.sambaaibot.slack.constants import FeedbackVisibility
from sambaai.sambaaibot.slack.models import ThreadMessage
from sambaai.prompts.miscellaneous_prompts import SLACK_LANGUAGE_REPHRASE_PROMPT
...
from sambaai.utils.text_processing import replace_whitespaces_w_space
from shared_configs.contextvars import CURRENT_TENANT_ID_CONTEXTVAR

logger = setup_logger()


_SAMBAAI_BOT_SLACK_BOT_ID: str | None = None
_SAMBAAI_BOT_MESSAGE_COUNT: int = 0
_SAMBAAI_BOT_COUNT_START_TIME: float = time.time()


def get_sambaai_bot_slack_bot_id(web_client: WebClient) -> Any:
    global _SAMBAAI_BOT_SLACK_BOT_ID
    if _SAMBAAI_BOT_SLACK_BOT_ID is None:
        _SAMBAAI_BOT_SLACK_BOT_ID = web_client.auth_test().get("user_id")
    return _SAMBAAI_BOT_SLACK_BOT_ID
...
def check_message_limit() -> bool:
    ...
    """
    if SAMBAAI_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD == 0:
        return True
    global _SAMBAAI_BOT_MESSAGE_COUNT
    global _SAMBAAI_BOT_COUNT_START_TIME
    time_since_start = time.time() - _SAMBAAI_BOT_COUNT_START_TIME
    if time_since_start > SAMBAAI_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS:
        _SAMBAAI_BOT_MESSAGE_COUNT = 0
        _SAMBAAI_BOT_COUNT_START_TIME = time.time()
    if (_SAMBAAI_BOT_MESSAGE_COUNT + 1) > SAMBAAI_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            f"SambaAIBot has reached the message limit {SAMBAAI_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD}"
            f" for the time period {SAMBAAI_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS} seconds."
            " These limits are configurable in backend/sambaai/configs/sambaaibot_configs.py"
        )
        return False
    _SAMBAAI_BOT_MESSAGE_COUNT += 1
    return True
...
def remove_sambaai_bot_tag(message_str: str, client: WebClient) -> str:
    bot_tag_id = get_sambaai_bot_slack_bot_id(web_client=client)
...
def _build_error_block(error_message: str) -> Block:
    ...
    """
    display_text = (
        "There was an error displaying all of the SambaAI answers."
        f" Please let an admin or an sambaai developer know. Error: {error_message}"
    )
    return SectionBlock(text=display_text)


@retry(
    tries=SAMBAAI_BOT_NUM_RETRIES,
    delay=0.25,
    backoff=2,
...
def read_slack_thread(
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            self_slack_bot_id = get_sambaai_bot_slack_bot_id(client)
            blocks: Any
            if reply.get("user") == self_slack_bot_id:
                ...
                # SambaAIBot responses have both text and blocks
                ...
                if not blocks:
                    logger.warning(f"SambaAIBot response has no blocks: {reply}")
                    continue
                ...
                if message is not None and message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the SambaAIBot answer format, if there is a change to how we respond,
                    # this will need to be updated to get the correct "answer" portion
                    message = reply["blocks"][1].get("text", {}).get("text")
            else:
                # Other bots are not counted as the LLM response which only comes from SambaAI
                message_type = MessageType.USER
        ...
        message = remove_sambaai_bot_tag(message, client=client)
...
def slack_usage_report(action: str, sender_id: str | None, client: WebClient) -> None:
    ...
    if sender_email is not None:
        with get_session_with_current_tenant() as db_session:
            sambaai_user = get_user_by_email(email=sender_email, db_session=db_session)

    optional_telemetry(
        record_type=RecordType.USAGE,
        data={"action": action},
        user_id=str(sambaai_user.id) if sambaai_user else "Non-SambaAI-Or-No-Auth-User",
    )


class SlackRateLimiter:
    def __init__(self) -> None:
        self.max_qpm: int | None = SAMBAAI_BOT_MAX_QPM
        self.max_wait_time = SAMBAAI_BOT_MAX_WAIT_TIME
...
def get_feedback_visibility() -> FeedbackVisibility:
    try:
        return FeedbackVisibility(SAMBAAI_BOT_FEEDBACK_VISIBILITY.lower())
    except ValueError:
