...
from sambaai.configs.app_configs import DEV_MODE
from sambaai.configs.app_configs import POD_NAME
from sambaai.configs.app_configs import POD_NAMESPACE
from sambaai.configs.constants import MessageType
from sambaai.configs.constants import SambaAIRedisLocks
from sambaai.configs.sambaaibot_configs import SAMBAAI_BOT_REPHRASE_MESSAGE
from sambaai.configs.sambaaibot_configs import SAMBAAI_BOT_RESPOND_EVERY_CHANNEL
from sambaai.configs.sambaaibot_configs import NOTIFY_SLACKBOT_NO_ANSWER
from sambaai.connectors.slack.utils import expert_info_from_slack_id
...
from sambaai.db.models import SlackBot
from sambaai.db.search_settings import get_current_search_settings
from sambaai.db.slack_bot import fetch_slack_bot
from sambaai.db.slack_bot import fetch_slack_bots
from sambaai.key_value_store.interface import KvKeyNotFoundError
from sambaai.natural_language_processing.search_nlp_models import EmbeddingModel
from sambaai.natural_language_processing.search_nlp_models import warm_up_bi_encoder
from sambaai.sambaaibot.slack.config import get_slack_channel_config_for_bot_and_channel
from sambaai.sambaaibot.slack.config import MAX_TENANTS_PER_POD
from sambaai.sambaaibot.slack.config import TENANT_ACQUISITION_INTERVAL
from sambaai.sambaaibot.slack.config import TENANT_HEARTBEAT_EXPIRATION
from sambaai.sambaaibot.slack.config import TENANT_HEARTBEAT_INTERVAL
from sambaai.sambaaibot.slack.config import TENANT_LOCK_EXPIRATION
from sambaai.sambaaibot.slack.constants import DISLIKE_BLOCK_ACTION_ID
from sambaai.sambaaibot.slack.constants import FEEDBACK_DOC_BUTTON_BLOCK_ACTION_ID
from sambaai.sambaaibot.slack.constants import FOLLOWUP_BUTTON_ACTION_ID
from sambaai.sambaaibot.slack.constants import FOLLOWUP_BUTTON_RESOLVED_ACTION_ID
from sambaai.sambaaibot.slack.constants import GENERATE_ANSWER_BUTTON_ACTION_ID
from sambaai.sambaaibot.slack.constants import IMMEDIATE_RESOLVED_BUTTON_ACTION_ID
from sambaai.sambaaibot.slack.constants import KEEP_TO_YOURSELF_ACTION_ID
from sambaai.sambaaibot.slack.constants import LIKE_BLOCK_ACTION_ID
from sambaai.sambaaibot.slack.constants import SHOW_EVERYONE_ACTION_ID
from sambaai.sambaaibot.slack.constants import VIEW_DOC_FEEDBACK_ID
from sambaai.sambaaibot.slack.handlers.handle_buttons import handle_doc_feedback_button
from sambaai.sambaaibot.slack.handlers.handle_buttons import handle_followup_button
from sambaai.sambaaibot.slack.handlers.handle_buttons import (
...
from sambaai.sambaaibot.slack.handlers.handle_buttons import handle_slack_feedback
from sambaai.sambaaibot.slack.handlers.handle_message import handle_message
from sambaai.sambaaibot.slack.handlers.handle_message import (
    remove_scheduled_feedback_reminder,
)
from sambaai.sambaaibot.slack.handlers.handle_message import schedule_feedback_reminder
from sambaai.sambaaibot.slack.models import SlackMessageInfo
from sambaai.sambaaibot.slack.utils import check_message_limit
from sambaai.sambaaibot.slack.utils import decompose_action_id
from sambaai.sambaaibot.slack.utils import get_channel_name_from_id
from sambaai.sambaaibot.slack.utils import get_sambaai_bot_slack_bot_id
from sambaai.sambaaibot.slack.utils import read_slack_thread
from sambaai.sambaaibot.slack.utils import remove_sambaai_bot_tag
from sambaai.sambaaibot.slack.utils import rephrase_slack_message
from sambaai.sambaaibot.slack.utils import respond_in_thread_or_channel
from sambaai.sambaaibot.slack.utils import TenantSocketModeClient
from sambaai.redis.redis_pool import get_redis_client
from sambaai.server.manage.models import SlackBotTokens
from sambaai.utils.logger import setup_logger
from sambaai.utils.variable_functionality import fetch_ee_implementation_or_noop
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        gated_tenants = fetch_ee_implementation_or_noop(
            "sambaai.server.tenants.product_gating",
            "get_gated_tenants",
            set(),
        )()
        all_tenants = [
        ...
        for tenant_id in all_tenants:
            ...
            rlock = redis_client.lock(
                SambaAIRedisLocks.SLACK_BOT_LOCK, timeout=TENANT_LOCK_EXPIRATION
            )
    ...
    def send_heartbeats(self) -> None:
        ...
        for tenant_id in self.tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = f"{SambaAIRedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{self.pod_id}"
            redis_client.set(
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    bot_tag_id = get_sambaai_bot_slack_bot_id(client.web_client)
    with get_session_with_current_tenant() as db_session:
    ...
    if req.type == "events_api":
        ...
        if (
            ...
            or remove_sambaai_bot_tag(msg, client=client.web_client)
        ...
        if event_type == "message":
            ...
            is_sambaai_bot_msg = bot_tag_id and bot_tag_id in event.get("user", "")

            # SambaAIBot should never respond to itself
            if is_sambaai_bot_msg:
                logger.info("Ignoring message from SambaAIBot")
                return False

            # DMs with the bot don't pick up the @SambaAIBot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If SambaAIBot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to SambaAI or @SambaAIBot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received SambaAIBot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to SambaAIBot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        msg = remove_sambaai_bot_tag(msg, client=client.web_client)

        if SAMBAAI_BOT_REPHRASE_MESSAGE:
        ...
        if tagged:
            logger.debug("User tagged SambaAIBot")
...
def process_message(
    ...
    client: TenantSocketModeClient,
    respond_every_channel: bool = SAMBAAI_BOT_RESPOND_EVERY_CHANNEL,
...
def _get_socket_client(
    ...
    # https://docs.sambaai.app/slack_bot_setup
