...
class SambaAIRedisSlackRetryHandler(RetryHandler):
    """
        ...
        *,
        ...
        try:
            if retry_after_header_name is None:
                ...
                raise ValueError(
                    "SambaAIRedisSlackRetryHandler.prepare_for_next_attempt: retry-after header name is None"
                )

            retry_after_value = response.headers.get(retry_after_header_name)
            if not retry_after_value:
                raise ValueError(
                    "SambaAIRedisSlackRetryHandler.prepare_for_next_attempt: retry-after header value is None"
                )
        ...
        lock: RedisLock = self._redis.lock(
            self._delay_lock,
            timeout=SambaAIRedisSlackRetryHandler.LOCK_TTL,
            thread_local=False,
        )

        acquired = lock.acquire(
            blocking_timeout=SambaAIRedisSlackRetryHandler.LOCK_BLOCKING_TIMEOUT / 2
        )
        ...
        logger.warning(
            f"SambaAIRedisSlackRetryHandler.prepare_for_next_attempt wait: "
