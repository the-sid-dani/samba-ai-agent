...
from sambaai.auth.email_utils import send_user_email_invite
from sambaai.auth.invited_users import get_invited_users
from sambaai.auth.invited_users import write_invited_users
from sambaai.auth.noauth_user import fetch_no_auth_user
from sambaai.auth.noauth_user import set_no_auth_user_preferences
...
from sambaai.auth.users import optional_user
from sambaai.configs.app_configs import AUTH_BACKEND
from sambaai.configs.app_configs import AUTH_TYPE
from sambaai.configs.app_configs import AuthBackend
from sambaai.configs.app_configs import DEV_MODE
from sambaai.configs.app_configs import ENABLE_EMAIL_INVITES
...
from sambaai.db.api_key import is_api_key_email_address
from sambaai.db.auth import get_live_users_count
from sambaai.db.engine import get_session
from sambaai.db.models import AccessToken
from sambaai.db.models import User
from sambaai.db.users import delete_user_from_db
from sambaai.db.users import get_all_users
from sambaai.db.users import get_page_of_filtered_users
from sambaai.db.users import get_total_filtered_users_count
from sambaai.db.users import get_user_by_email
from sambaai.db.users import validate_user_role_update
from sambaai.key_value_store.factory import get_kv_store
from sambaai.redis.redis_pool import get_raw_redis_client
from sambaai.server.documents.models import PaginatedReturn
from sambaai.server.manage.models import AllUsersResponse
from sambaai.server.manage.models import AutoScrollRequest
from sambaai.server.manage.models import TenantInfo
from sambaai.server.manage.models import TenantSnapshot
from sambaai.server.manage.models import UserByEmail
from sambaai.server.manage.models import UserInfo
from sambaai.server.manage.models import UserPreferences
from sambaai.server.manage.models import UserRoleResponse
from sambaai.server.manage.models import UserRoleUpdateRequest
from sambaai.server.models import FullUserSnapshot
from sambaai.server.models import InvitedUserSnapshot
from sambaai.server.models import MinimalUserSnapshot
...
from sambaai.utils.variable_functionality import (
...
async def test_upsert_user(
    ...
    user = await fetch_ee_implementation_or_noop(
        "sambaai.server.saml", "upsert_saml_user", None
    )(email=request.email)
...
def bulk_invite_users(
    ...
    if MULTI_TENANT:
        try:
            fetch_ee_implementation_or_noop(
                "sambaai.server.tenants.provisioning", "add_users_to_tenant", None
    ...
    try:
        ...
        fetch_ee_implementation_or_noop(
            "sambaai.server.tenants.billing", "register_tenant_users", None
        ...
        fetch_ee_implementation_or_noop(
            "sambaai.server.tenants.user_mapping", "remove_users_from_tenant", None
        )(new_invited_emails, tenant_id)
        raise e
    ...
    if MULTI_TENANT:
        if team_name != get_current_tenant_id():
            user_count = fetch_ee_implementation_or_noop(
                "sambaai.server.tenants.user_mapping", "get_tenant_count", None
            )(team_name)
        ...
        tenant_invitation = fetch_ee_implementation_or_noop(
            "sambaai.server.tenants.user_mapping", "get_tenant_invitation", None
        )(user.email)

    super_users_list = cast(
        list[str],
        fetch_versioned_implementation_with_fallback(
            "sambaai.configs.app_configs",
            "SUPER_USERS",
            [],
        ),
    )
