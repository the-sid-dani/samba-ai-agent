...
from sambaai.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from sambaai.background.celery.tasks.beat_schedule import (
    CLOUD_DOC_PERMISSION_SYNC_MULTIPLIER_DEFAULT,
)
from sambaai.configs.constants import CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT
from sambaai.configs.constants import ONYX_CLOUD_REDIS_RUNTIME
from sambaai.configs.constants import ONYX_CLOUD_TENANT_ID
from sambaai.configs.constants import ONYX_EMAILABLE_LOGO_MAX_DIM
...
from sambaai.utils.file import FileWithMimeType
from sambaai.utils.file import SambaAIStaticFileManager
from sambaai.utils.variable_functionality import (
    fetch_ee_implementation_or_noop,
)


class SambaAIRuntime:
    ...
    def _get_with_static_fallback(
        ...
        sambaai_file: FileWithMimeType | None = None

        if db_filename:
            with get_session_with_shared_schema() as db_session:
                file_store = PostgresBackedFileStore(db_session)
                sambaai_file = file_store.get_file_with_mime_type(db_filename)

        if not sambaai_file:
            sambaai_file = SambaAIStaticFileManager.get_static(static_filename)
    ...
    def get_logo() -> FileWithMimeType:
        ...
        db_filename: str | None = fetch_ee_implementation_or_noop(
            "sambaai.server.enterprise_settings.store", "get_logo_filename", None
        )

        return SambaAIRuntime._get_with_static_fallback(db_filename, STATIC_FILENAME)

    @staticmethod
    def get_emailable_logo() -> FileWithMimeType:
        sambaai_file = SambaAIRuntime.get_logo()

        # check dimensions and resize downwards if necessary or if not PNG
        image = Image.open(io.BytesIO(sambaai_file.data))
        if (
    ...
    def get_logotype() -> FileWithMimeType:
        ...
        db_filename: str | None = fetch_ee_implementation_or_noop(
            "sambaai.server.enterprise_settings.store", "get_logotype_filename", None
        )
