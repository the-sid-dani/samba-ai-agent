...
from sambaai.server.onyx_api.ingestion import api_key_dep
...
PUBLIC_ENDPOINT_SPECS = [
    ...
    ("/auth/type", {"GET"}),
    # just gets the version of SambaAI (e.g. 0.3.11)
    ("/version", {"GET"}),
...
def check_router_auth(
    ...
    control_plane_dep = fetch_ee_implementation_or_noop(
        "sambaai.server.tenants.access", "control_plane_dep"
    )
    current_cloud_superuser = fetch_ee_implementation_or_noop(
        "sambaai.auth.users", "current_cloud_superuser"
    )
