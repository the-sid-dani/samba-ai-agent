...
def add_sambaai_tenant_id_middleware(app: FastAPI, logger: logging.LoggerAdapter) -> None:
    @app.middleware("http")
    async def set_tenant_id(
        ...
        sambaai_tenant_id = request.headers.get("X-SambaAI-Tenant-ID")
        if sambaai_tenant_id:
            CURRENT_TENANT_ID_CONTEXTVAR.set(sambaai_tenant_id)
        return await call_next(request)


def add_sambaai_request_id_middleware(
    ...
    async def set_request_id(
        ...
        sambaai_request_id = request.headers.get("X-SambaAI-Request-ID")
        if not sambaai_request_id:
            sambaai_request_id = make_randomized_sambaai_request_id(prefix)

        ONYX_REQUEST_ID_CONTEXTVAR.set(sambaai_request_id)
        return await call_next(request)


def make_randomized_sambaai_request_id(prefix: str) -> str:
    """generates a randomized request id"""

    hash_input = str(uuid.uuid4())
    return _make_sambaai_request_id(prefix, hash_input)


def make_structured_sambaai_request_id(prefix: str, request_url: str) -> str:
    """Not used yet, but could be in the future!"""
    hash_input = f"{request_url}:{datetime.now(timezone.utc)}"
    return _make_sambaai_request_id(prefix, hash_input)


def _make_sambaai_request_id(prefix: str, hash_input: str) -> str:
    ...
    sambaai_request_id = f"{prefix}:{hash_str}"
    return sambaai_request_id
