...
from sambaai.chat.stream_processing.utils import DocumentIdOrderMapping
from sambaai.configs.chat_configs import STOP_STREAM_PAT
from sambaai.prompts.constants import TRIPLE_BACKTICK
...
class CitationProcessor:
    ...
    def process_token(
        self, token: str | None
    ) -> Generator[SambaAIAnswerPiece | CitationInfo, None, None]:
        # None -> end of stream
        if token is None:
            yield SambaAIAnswerPiece(answer_piece=self.curr_segment)
            return
        ...
        if result:
            yield SambaAIAnswerPiece(answer_piece=result)
