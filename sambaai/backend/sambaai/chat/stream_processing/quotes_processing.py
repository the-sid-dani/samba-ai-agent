...
from sambaai.chat.models import SambaAIAnswer
from sambaai.chat.models import SambaAIAnswerPiece
from sambaai.configs.chat_configs import QUOTE_ALLOWED_ERROR_PERCENT
from sambaai.context.search.models import InferenceChunk
from sambaai.prompts.constants import ANSWER_PAT
from sambaai.prompts.constants import QUOTE_PAT
from sambaai.utils.logger import setup_logger
from sambaai.utils.text_processing import clean_model_quote
from sambaai.utils.text_processing import clean_up_code_blocks
from sambaai.utils.text_processing import extract_embedded_json
from sambaai.utils.text_processing import shared_precompare_cleanup
...
class SambaAIQuotes(BaseModel):
    quotes: list[SambaAIQuote]
...
def match_quotes_to_docs(
    ...
    sambaai_quotes: list[SambaAIQuote] = []
    for quote in quotes:
    ...
    return SambaAIQuotes(quotes=sambaai_quotes)
...
def _process_answer(
...
) -> tuple[SambaAIAnswer, SambaAIQuotes]:
    ...
    if not answer:
        ...
        return SambaAIAnswer(answer=None), SambaAIQuotes(quotes=[])

    logger.notice(f"Answer: {answer}")
    if not quote_strings:
        logger.debug("No quotes extracted from raw output")
        return SambaAIAnswer(answer=answer), SambaAIQuotes(quotes=[])
    ...
    return SambaAIAnswer(answer=answer), quotes
...
class QuotesProcessor:
    ...
    def process_token(
        self, token: str | None
    ) -> Generator[SambaAIAnswerPiece | SambaAIQuotes, None, None]:
        # None -> end of stream
        if token is None:
            ...
            if m:
                ...
                for quote_idx in quote_indices:
                    ...
                    if num_backslashes % 2 == 0:
                        yield SambaAIAnswerPiece(answer_piece=remaining[:quote_idx])
                        return

                # If no unescaped quote found, yield the remaining string
                if len(remaining) > 0:
                    yield SambaAIAnswerPiece(answer_piece=remaining)
                return
            ...
            if self.is_json_prompt and _stream_json_answer_end(model_previous, token):
                ...
                if token:
                    try:
                        ...
                        yield SambaAIAnswerPiece(
                            answer_piece=self.hold_quote + token[:answer_token_section]
                        )
                    except ValueError:
                        logger.error("Quotation mark not found in token")
                        yield SambaAIAnswerPiece(answer_piece=self.hold_quote + token)
                yield SambaAIAnswerPiece(answer_piece=None)
                return
