...
from sambaai.access.utils import prefix_user_email
from sambaai.configs.constants import PUBLIC_DOC_PAT
from sambaai.db.document import get_access_info_for_document
from sambaai.db.document import get_access_info_for_documents
...
def get_access_for_document(
    ...
    versioned_get_access_for_document_fn = fetch_versioned_implementation(
        "sambaai.access.access", "_get_access_for_document"
    )
...
def get_access_for_documents(
    ...
    versioned_get_access_for_documents_fn = fetch_versioned_implementation(
        "sambaai.access.access", "_get_access_for_documents"
    )
...
def get_acl_for_user(user: User | None, db_session: Session | None = None) -> set[str]:
    versioned_acl_for_user_fn = fetch_versioned_implementation(
        "sambaai.access.access", "_get_acl_for_user"
    )
