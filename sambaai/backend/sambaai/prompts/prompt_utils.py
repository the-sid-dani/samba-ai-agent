...
_SAMBAAI_DATETIME_REPLACEMENT_PAT = "[[CURRENT_DATETIME]]"
...
def handle_sambaai_date_awareness(
    prompt_str: str,
    ...
    if _SAMBAAI_DATETIME_REPLACEMENT_PAT in prompt_str:
        return prompt_str.replace(
            _SAMBAAI_DATETIME_REPLACEMENT_PAT,
            get_current_llm_day_time(full_sentence=False, include_day_of_week=True),
        )
    any_tag_present = any(
        _SAMBAAI_DATETIME_REPLACEMENT_PAT in text
