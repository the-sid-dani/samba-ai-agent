...
from sambaai.configs.chat_configs import VESPA_SEARCHER_THREADS
from sambaai.configs.constants import KV_REINDEX_KEY
from sambaai.context.search.models import IndexFilters
from sambaai.context.search.models import InferenceChunkUncleaned
from sambaai.db.enums import EmbeddingPrecision
from sambaai.document_index.document_index_utils import get_document_chunk_ids
from sambaai.document_index.interfaces import DocumentIndex
from sambaai.document_index.interfaces import DocumentInsertionRecord
from sambaai.document_index.interfaces import EnrichedDocumentIndexingInfo
from sambaai.document_index.interfaces import IndexBatchParams
from sambaai.document_index.interfaces import MinimalDocumentIndexingInfo
from sambaai.document_index.interfaces import UpdateRequest
from sambaai.document_index.interfaces import VespaChunkRequest
from sambaai.document_index.interfaces import VespaDocumentFields
from sambaai.document_index.interfaces import VespaDocumentUserFields
from sambaai.document_index.vespa.chunk_retrieval import batch_search_api_retrieval
from sambaai.document_index.vespa.chunk_retrieval import (
    parallel_visit_api_retrieval,
)
from sambaai.document_index.vespa.chunk_retrieval import query_vespa
from sambaai.document_index.vespa.deletion import delete_vespa_chunks
from sambaai.document_index.vespa.indexing_utils import BaseHTTPXClientContext
from sambaai.document_index.vespa.indexing_utils import batch_index_vespa_chunks
from sambaai.document_index.vespa.indexing_utils import check_for_final_chunk_existence
from sambaai.document_index.vespa.indexing_utils import clean_chunk_id_copy
from sambaai.document_index.vespa.indexing_utils import GlobalHTTPXClientContext
from sambaai.document_index.vespa.indexing_utils import TemporaryHTTPXClientContext
from sambaai.document_index.vespa.shared_utils.utils import get_vespa_http_client
from sambaai.document_index.vespa.shared_utils.utils import (
    replace_invalid_doc_id_characters,
)
from sambaai.document_index.vespa.shared_utils.vespa_request_builders import (
    build_vespa_filters,
)
from sambaai.document_index.vespa_constants import ACCESS_CONTROL_LIST
from sambaai.document_index.vespa_constants import BATCH_SIZE
from sambaai.document_index.vespa_constants import BOOST
from sambaai.document_index.vespa_constants import CONTENT_SUMMARY
from sambaai.document_index.vespa_constants import DOCUMENT_ID_ENDPOINT
from sambaai.document_index.vespa_constants import DOCUMENT_SETS
from sambaai.document_index.vespa_constants import HIDDEN
from sambaai.document_index.vespa_constants import NUM_THREADS
from sambaai.document_index.vespa_constants import USER_FILE
from sambaai.document_index.vespa_constants import USER_FOLDER
from sambaai.document_index.vespa_constants import VESPA_APPLICATION_ENDPOINT
from sambaai.document_index.vespa_constants import VESPA_TIMEOUT
from sambaai.document_index.vespa_constants import YQL_BASE
from sambaai.indexing.models import DocMetadataAwareIndexChunk
from sambaai.key_value_store.factory import get_shared_kv_store
from sambaai.utils.batching import batch_generator
...
class VespaIndex(DocumentIndex):
    ...
    VESPA_SCHEMA_JINJA_FILENAME = "sambaai_chunk.sd.jinja"
    ...
    def ensure_indices_exist(
        ...
        vespa_schema_path = os.path.join(
            os.getcwd(), "sambaai", "document_index", "vespa", "app_config"
        )
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to prepare Vespa SambaAI Index. Response: {response.text}"
            )
            raise RuntimeError(
                f"Failed to prepare Vespa SambaAI Index. Response: {response.text}"
            )
    ...
    def register_multitenant_indices(
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"Failed to prepare Vespa SambaAI Indexes. Response: {response.text}"
            )
