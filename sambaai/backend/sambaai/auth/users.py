...
from sambaai.auth.api_key import get_hashed_api_key_from_request
from sambaai.auth.email_utils import send_forgot_password_email
from sambaai.auth.email_utils import send_user_verification_email
from sambaai.auth.invited_users import get_invited_users
from sambaai.auth.schemas import AuthBackend
from sambaai.auth.schemas import UserCreate
from sambaai.auth.schemas import UserRole
from sambaai.auth.schemas import UserUpdateWithRole
from sambaai.configs.app_configs import AUTH_BACKEND
from sambaai.configs.app_configs import AUTH_COOKIE_EXPIRE_TIME_SECONDS
from sambaai.configs.app_configs import AUTH_TYPE
from sambaai.configs.app_configs import DISABLE_AUTH
from sambaai.configs.app_configs import EMAIL_CONFIGURED
from sambaai.configs.app_configs import REDIS_AUTH_KEY_PREFIX
from sambaai.configs.app_configs import REQUIRE_EMAIL_VERIFICATION
from sambaai.configs.app_configs import SESSION_EXPIRE_TIME_SECONDS
...
from sambaai.configs.app_configs import VALID_EMAIL_DOMAINS
from sambaai.configs.app_configs import WEB_DOMAIN
from sambaai.configs.constants import ANONYMOUS_USER_COOKIE_NAME
from sambaai.configs.constants import AuthType
from sambaai.configs.constants import SAMBAAI_API_KEY_DUMMY_EMAIL_DOMAIN
from sambaai.configs.constants import SAMBAAI_API_KEY_PREFIX
from sambaai.configs.constants import FASTAPI_USERS_AUTH_COOKIE_NAME
from sambaai.configs.constants import MilestoneRecordType
from sambaai.configs.constants import SambaAIRedisLocks
from sambaai.configs.constants import PASSWORD_SPECIAL_CHARS
from sambaai.configs.constants import UNNAMED_KEY_PLACEHOLDER
from sambaai.db.api_key import fetch_user_for_api_key
from sambaai.db.auth import get_access_token_db
from sambaai.db.auth import get_default_admin_user_emails
from sambaai.db.auth import get_user_count
from sambaai.db.auth import get_user_db
from sambaai.db.auth import SQLAlchemyUserAdminDB
from sambaai.db.engine import get_async_session
from sambaai.db.engine import get_async_session_context_manager
from sambaai.db.engine import get_session_with_tenant
from sambaai.db.models import AccessToken
from sambaai.db.models import OAuthAccount
from sambaai.db.models import User
from sambaai.db.users import get_user_by_email
from sambaai.redis.redis_pool import get_async_redis_connection
...
from sambaai.utils.url import add_url_params
...
def get_display_email(email: str | None, space_less: bool = False) -> str:
    if email and email.endswith(SAMBAAI_API_KEY_DUMMY_EMAIL_DOMAIN):
        name = email.split("@")[0]
        if name == SAMBAAI_API_KEY_PREFIX + UNNAMED_KEY_PLACEHOLDER:
...
def anonymous_user_enabled(*, tenant_id: str | None = None) -> bool:
    ...
    value = redis_client.get(SambaAIRedisLocks.ANONYMOUS_USER_ENABLED)
...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def get_by_email(self, user_email: str) -> User:
        tenant_id = fetch_ee_implementation_or_noop(
            "sambaai.server.tenants.user_mapping", "get_tenant_id_for_email", None
        )(user_email)
    ...
    async def create(
        ...
        tenant_id = await fetch_ee_implementation_or_noop(
            "sambaai.server.tenants.provisioning",
...
class FastAPIUserWithLogoutRouter(FastAPIUsers[models.UP, models.ID]):
    ...
    def get_refresh_router(
        ...
        from sambaai.auth.oauth_refresher import check_and_refresh_oauth_tokens
...
async def optional_user(
    ...
    versioned_fetch_user = fetch_versioned_implementation(
        "sambaai.auth.users", "optional_user_"
    )
...
def get_default_admin_user_emails_() -> list[str]:
    # No default seeding available for SambaAI MIT
    return []
...
def create_sambaai_oauth_router(
