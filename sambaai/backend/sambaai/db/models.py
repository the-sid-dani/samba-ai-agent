...
from sambaai.configs.chat_configs import NUM_POSTPROCESSED_RESULTS
from sambaai.configs.constants import DEFAULT_BOOST, MilestoneRecordType
...
from sambaai.configs.constants import TokenRateLimitScope
from sambaai.connectors.models import InputType
from sambaai.db.enums import ChatSessionSharedStatus
...
from sambaai.db.pydantic_type import PydanticType
...
from sambaai.context.search.enums import RecencyBiasSetting
from sambaai.utils.encryption import decrypt_bytes_to_string
from sambaai.utils.encryption import encrypt_string_to_bytes
from sambaai.utils.headers import HeaderItemDict
...
class ConnectorCredentialPair(Base):
    ...
    # {"customer_id": "123567", "company_domain": "@sambaai.app"}
...
class Document(Base):
    ...
    # (as is passed around in SambaAI)
...
class ChatSession(Base):
    ...
    # This chat created by SambaAIBot
    sambaaibot_flow: Mapped[bool] = mapped_column(Boolean, default=False)
...
These models are only used in Enterprise Edition only features in SambaAI.
They are kept here to simplify the codebase and avoid having different assumptions
on the shape of data being passed around between the MIT and EE versions of SambaAI.

In the MIT version of SambaAI, assume these tables are always empty.
"""
...
class User__ExternalUserGroupId(Base):
    ...
    directly adding the SambaAI user to the doc ACL list"""
