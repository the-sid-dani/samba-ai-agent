...
from sambaai.db.api_key import SAMBAAI_API_KEY_DUMMY_EMAIL_DOMAIN
from sambaai.db.models import DocumentSet__User
from sambaai.db.models import Persona__User
from sambaai.db.models import SamlAccount
...
def validate_user_role_update(
    ...
    if current_role == UserRole.SLACK_USER:
        raise HTTPException(
            status_code=400,
            detail="To change a Slack User's role, they must first login to SambaAI via the web app.",
        )
    ...
    if current_role == UserRole.EXT_PERM_USER:
        ...
        raise HTTPException(
            status_code=400,
            detail="To change an External Permissioned User's role, they must first login to SambaAI via the web app.",
        )

    if current_role == UserRole.LIMITED:
        raise HTTPException(
            status_code=400,
            detail="To change a Limited User's role, they must first login to SambaAI via the web app.",
        )
    ...
    if requested_role == UserRole.SLACK_USER:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "This role is automatically assigned to users who only use SambaAI via Slack."
            ),
        )
...
def _get_accepted_user_where_clause(
    ...
    where_clause: list[ColumnElement[bool]] = [
        expression.not_(email_col.endswith(SAMBAAI_API_KEY_DUMMY_EMAIL_DOMAIN))
    ]
