...
from sambaai.connectors.sambaai_jira.connector import JiraConnectorCheckpoint
...
def jira_connector(
    ...
    with patch("sambaai.connectors.sambaai_jira.connector._JIRA_FULL_PAGE_SIZE", 2):
        yield connector
...
def test_load_credentials(jira_connector: JiraConnector) -> None:
    ...
    with patch(
        "sambaai.connectors.sambaai_jira.connector.build_jira_client"
...
def test_load_from_checkpoint_with_issue_processing_error(
    ...
    with patch(
        "sambaai.connectors.sambaai_jira.connector.process_jira_issue"
    ) as mock_process:
...
def test_retrieve_all_slim_documents(
    ...
    with patch(
        "sambaai.connectors.sambaai_jira.connector.best_effort_get_field_from_issue"
    ) as mock_field:
        mock_field.side_effect = ["TEST-1", "TEST-2"]

        # Mock build_jira_url to return URLs
        with patch("sambaai.connectors.sambaai_jira.connector.build_jira_url") as mock_url:
