...
1. Launch sambaai (using Docker or running with a debugger), ensuring the API server is running on port 8080.
   - If you'd like to set environment variables, you can do so by creating a `.env` file in the sambaai/backend/tests/integration/ directory.
   - SambaAI MUST be launched with AUTH_TYPE=basic and ENABLE_PAID_ENTERPRISE_EDITION_FEATURES=true
2. Navigate to `sambaai/backend`.
...
sambaai services with a name different from the default `sambaai-stack`.
