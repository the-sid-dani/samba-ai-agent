...
def test_slack_permission_sync(
    ...
    sambaai_doc_message_strings = DocumentSearchManager.search_documents(
        query="favorite number",
        user_performing_action=admin_user,
    )
    print(
        "\n documents retrieved by admin user: ",
        sambaai_doc_message_strings,
    )

    # Ensure admin user can see messages from both channels
    assert public_message in sambaai_doc_message_strings
    assert private_message in sambaai_doc_message_strings
    ...
    assert public_message in sambaai_doc_message_strings
    assert private_message not in sambaai_doc_message_strings
...
def test_slack_group_permission_sync(
    ...
    """
    This test ensures that permission sync overrides sambaai group access.
    """
    public_channel, private_channel = slack_test_setup
