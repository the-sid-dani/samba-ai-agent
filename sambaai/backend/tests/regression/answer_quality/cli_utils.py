...
def manage_data_directories(env_name: str, base_path: str, use_cloud_gpu: bool) -> None:
    ...
    directories = {
        "SAMBAAI_POSTGRES_DATA_DIR": os.path.join(target_path, "postgres/"),
        "SAMBAAI_VESPA_DATA_DIR": os.path.join(target_path, "vespa/"),
    }
    if not use_cloud_gpu:
        directories["SAMBAAI_INDEX_MODEL_CACHE_DIR"] = os.path.join(
            target_path, "index_model_cache/"
        )
        directories["SAMBAAI_INFERENCE_MODEL_CACHE_DIR"] = os.path.join(
...
def start_docker_compose(
    ...
    command = f"docker compose -f docker-compose.search-testing.yml -p sambaai-stack-{env_name} up -d"
    command += " --build"
...
def cleanup_docker(env_name: str) -> None:
    ...
    project_name = f"sambaai-stack{env_name}"
...
if __name__ == "__main__":
    ...
    If no environment_name is indicated, will just clean up all sambaai docker containers/volumes/networks
