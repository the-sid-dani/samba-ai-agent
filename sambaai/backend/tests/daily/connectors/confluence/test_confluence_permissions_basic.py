...
def confluence_connector() -> ConfluenceConnector:
    connector = ConfluenceConnector(
        wiki_base="https://sambaaiai.atlassian.net",
        is_cloud=True,
    )

    credentials_provider = SambaAIStaticCredentialsProvider(
        None,
...
@patch("ee.sambaai.external_permissions.confluence.doc_sync.SambaAIDBCredentialsProvider")
@patch(
    "sambaai.file_processing.extract_file_text.get_unstructured_api_key",
    return_value=None,
)
def test_confluence_connector_restriction_handling(
    ...
    # this prevents redis calls inside of SambaAIConfluence
    ...
    non_restricted_emails = {"chris@sambaai.app"}
    non_restricted_user_groups = {
        "confluence-admins-sambaaiai",
        "org-admins",
        "atlassian-addons-admin",
        "confluence-users-sambaaiai",
    }

    # if restriction is applied, only should be visible to shared users / groups
    restricted_emails = {"chris@sambaai.app", "hagen@sambaai.ai"}
    restricted_user_groups = {"confluence-admins-sambaaiai"}

    extra_restricted_emails = {"chris@sambaai.app"}
    extra_restricted_user_groups: set[str] = set()

    # note that this is only allowed since yuhong@sambaai.app is a member of the
    # confluence-admins-sambaaiai group
    special_restricted_emails = {"chris@sambaai.app", "yuhong@sambaai.app"}
