...
def jira_connector() -> JiraConnector:
    connector = JiraConnector(
        jira_base_url="https://sambaaiai.atlassian.net",
        project_key="AS",
...
def test_jira_connector_basic(
    ...
    # Check task
    assert story.id == "https://sambaaiai.atlassian.net/browse/AS-3"
    ...
    assert section.link == "https://sambaaiai.atlassian.net/browse/AS-3"

    # Check epic
    assert epic.id == "https://sambaaiai.atlassian.net/browse/AS-4"
    assert epic.semantic_identifier == "AS-4: EPIC"
    assert epic.source == DocumentSource.JIRA
    assert epic.metadata == {
        "priority": "Medium",
        "status": "Backlog",
        "reporter": "Founder SambaAI",
        "assignee": "Chris Weaver",
        "issuetype": "Epic",
    ...
    assert section.text == "example_text\n"
    assert section.link == "https://sambaaiai.atlassian.net/browse/AS-4"
