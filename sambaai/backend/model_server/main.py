...
from sambaai.utils.middleware import add_sambaai_tenant_id_middleware
...
def get_model_app() -> FastAPI:
    application = FastAPI(
        title="SambaAI Model Server", version=__version__, lifespan=lifespan
    )
    if SENTRY_DSN:
        sentry_sdk.init(
            dsn=SENTRY_DSN,
    ...
    add_sambaai_tenant_id_middleware(application, logger)
    add_sambaai_request_id_middleware(application, request_id_prefix, logger)
...
if __name__ == "__main__":
    logger.notice(
        f"Starting SambaAI Model Server on http://{MODEL_SERVER_ALLOWED_HOST}:{str(MODEL_SERVER_PORT)}/"
    )
