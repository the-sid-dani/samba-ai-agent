...
from sambaai.configs.app_configs import REDIS_DB_NUMBER
from sambaai.configs.app_configs import REDIS_HOST
from sambaai.configs.app_configs import REDIS_PASSWORD
from sambaai.configs.app_configs import REDIS_PORT
from sambaai.configs.app_configs import REDIS_SSL
...
from sambaai.redis.redis_pool import RedisPool
...
class SambaAIRedisCommand(Enum):
...
def sambaai_redis(
    ...
    if command == SambaAIRedisCommand.purge_connectorsync_taskset:
    ...
    elif command == SambaAIRedisCommand.purge_documentset_taskset:
        return purge_by_match_and_type(
            "*documentset_taskset*", "set", batch, dry_run, r
        )
    elif command == SambaAIRedisCommand.purge_usergroup_taskset:
        return purge_by_match_and_type("*usergroup_taskset*", "set", batch, dry_run, r)
    elif command == SambaAIRedisCommand.purge_locks_blocking_deletion:
        ...
        return 0
    elif command == SambaAIRedisCommand.purge_vespa_syncing:
        return purge_by_match_and_type(
            "*connectorsync:vespa_syncing*", "string", batch, dry_run, r
        )
    elif command == SambaAIRedisCommand.purge_pidbox:
        return purge_by_match_and_type(
            "*reply.celery.pidbox", "list", batch, dry_run, r
        )
    elif command == SambaAIRedisCommand.get_list_element:
        ...
        print(f"{result}")
        return 0
    elif command == SambaAIRedisCommand.get_user_token:
        if not user_email:
        ...
        if token_key:
            ...
            return 2
    elif command == SambaAIRedisCommand.delete_user_token:
        if not user_email:
        ...
        if delete_user_token_from_redis(r, user_email, dry_run):
            ...
            return 2
    elif command == SambaAIRedisCommand.add_invited_user:
        if not user_email:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SambaAI Redis Manager")
    parser.add_argument(
        "--command",
        type=SambaAIRedisCommand,
        help="The command to run",
        choices=list(SambaAIRedisCommand),
        required=True,
    )
