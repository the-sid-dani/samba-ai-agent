...
# In this case, the equivalent of asking a question in SambaAI Chat in a new chat session
import argparse
import json
import os
...
def process_question(sambaai_url: str, question: str, api_key: str | None) -> None:
    message_endpoint = sambaai_url + "/api/chat/send-message"

    chat_session_id = create_new_chat_session(sambaai_url, api_key)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test-question",
        type=str,
        default="What is SambaAI?",
        help="Test question for new Chat Session.",
    )

    # Not needed if Auth is disabled
    # Or for SambaAI MIT API key must be replaced with session cookie
    api_key = os.environ.get("SAMBAAI_API_KEY")

    args = parser.parse_args()
    process_question(
        sambaai_url=args.sambaai_url, question=args.test_question, api_key=api_key
    )
