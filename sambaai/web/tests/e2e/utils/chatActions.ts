...
export async function sendMessage(page: Page, message: string) {
  await page.locator("#sambaai-chat-input-textarea").click();
  await page.locator("#sambaai-chat-input-textarea").fill(message);
  await page.locator("#sambaai-chat-input-send-button").click();
  await page.waitForSelector("#sambaai-ai-message");
  await page.waitForTimeout(2000);
}

export async function verifyCurrentModel(page: Page, modelName: string) {
  await page.waitForTimeout(1000);
  const chatInput = page.locator("#sambaai-chat-input");
