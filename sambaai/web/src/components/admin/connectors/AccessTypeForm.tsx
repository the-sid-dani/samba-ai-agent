...
export function AccessTypeForm({
  ...
  if (isAdmin) {
    options.push({
      ...
      value: "public",
      description:
        "Everyone with an account on SambaAI can access the documents pulled in by this connector",
    });
  }
  ...
  if (isAutoSyncSupported && isPaidEnterpriseEnabled) {
    options.push({
      ...
      value: "sync",
      description:
        "We will automatically sync permissions from the source. A document will be searchable in SambaAI if and only if the user performing the search has permission to access the document in the source.",
    });
  }
